...
After opening twitter, I came across a tweet discussing John Helvy's 
...
Then we create a dataframe as an input for the game. 
In this dataframe the `alive` column is set to 1 as in the beginning
obviously every player is alive.
...
sims <- tibble(trial = seq(n_runs)) |> 
  mutate(while_loop = map_dbl(trial, ~run_game_while(players, num_steps = 18)),
         for_loop = map_dbl(trial, ~run_game_for(players, num_steps = 18))) |> 
...
There were games where zero players survived the game and there were games
where almost all players managed to survive.
...
The two different colours indicate which function was used to calculate the
result. From this graph we directly see almost the exact same result from the 
two functions. 
...
for (i in 0:(n-1)) {
  res[i+1] <- choose(18, i) * 0.5^i * 0.5^(18-i) * (n - i)
}
...
Et voilà! We would expect 7 players to survive the game.
